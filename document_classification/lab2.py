...
а втор елемент е класата како стринг. Да се истренира модел врз основа на тренинг множеството. Потоа, за секој прочитан
документ од стандарден влез да се испечатат зборовите кои се употребуваат за класификација, класата и веројатноста на
зборот да е од таа класа (заокружено на 5 децимали), како и тежинската веројатност на зборот да припаѓа на класата
(заокружено на 5 децимали). На крај да се испечати предвидената класа на документот и логаритам со основа 2 од
веројатноста со која се предвидува (заокружено на 5 децимали). Забелешка: зборовите подредете ги по азбучен редослед.
"""
...
if __name__ == "__main__":
    ...
    # when we don't specify the 'reverse' argument of 'sorted', it's 'False' by default
    words = sorted(get_words(test_doc))

    for word in words:
        # key point: 'веројатноста на зборот да е од таа класа'
        prob = classifier.get_feature_per_category_probability(word, "science")
        # key point: 'тежинската веројатност'
        weighted_prob = classifier.weighted_probability(word, "science", classifier.get_feature_per_category_probability)
        print(word, "science", round(prob, 5), round(weighted_prob, 5))

        prob = classifier.get_feature_per_category_probability(word, "sport")
        weighted_prob = classifier.weighted_probability(word, "sport", classifier.get_feature_per_category_probability)
        print(word, "sport", round(prob, 5), round(weighted_prob, 5))

    pred = classifier.classify_document(test_doc)
    # key point: 'предвидената класа на документот и логаритам со основа 2 од веројатноста со која се предвидува'
    prob = classifier.get_category_probability_for_document(test_doc, pred)
    print(pred, round(log2(prob), 5))
