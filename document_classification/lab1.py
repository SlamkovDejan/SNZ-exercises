...
if __name__ == "__main__":
    ...
    # first we make an object from the "NaiveBayes" class
    # the constructor of "NaiveBayes" takes a function as an argument
    # that function is the one that splits the documents into set of words (set of features)
    classifier = NaiveBayes(get_words)

    # next we must train the model with every document in train_data
    for doc in train_data:
        # we use the 'train' function from "DocumentClassifier"
        # the first argument of 'train' is the document, the second argument is the class of that document
        # doc[0] -> document; doc[1] -> class of doc[0]
        classifier.train(doc[0], doc[1])

    # we use 'classify_document' from 'NaiveBayes' to predict a class for a certain document
    # 'classify_document' returns one class (one of the classes in the train set, in this case it will return 'sport' or 'science')
    pred = classifier.classify_document(recenica)
    print(pred)

    # key point: 'веројатноста за одредената класа'
    prob = classifier.get_category_probability_for_document(recenica, pred)
