"""
Потребно е да изградите два класификатори со Наивен Баес за класификација на документи кои се постови од социјалната
мрежа Twitter. Класите на документите се позитивен или негативен сентимент. Податоците се дадени во променливата data,
како торки каде што првиот елемент е класата, а вториот елемент е документот. Од влез во променливата sample_ind се
сместува индексот на документот кој треба да се предвиди со креираните модели. Овој документ се вади од множеството и
служи за тестирање, а остатокот е множество за тренирање.

Првиот класификатор треба да ја користи стандарната функција за делба на зборови get_words која е дадена на
аудиториските вежби. Вториот класификатор треба да користи втора верзија на оваа функција која дополнително ќе вклучува
емотикони од текстот. Оваа нова функција треба да ја користи истата логика за извлекување на зборовите како и функцијата
get_words, каде што потоа дополнително се прави проверка на емотиконите. За да ги добиете емотиконите потребно е
дадениот документ да се подели по празни места. За да го поделите документот според празни места користете ја функцијата
split дефинирана на документи во кој аргументот кој ќе го предадете е стринг составен од празно место
(пример: document.split(' ')). Потоа, во резултатот кој се враќа од функцијата потребно е да се вклучат зборовите од
кои е составен документот и емотиконите кои ги содржи (се вклучуваат само емотиконот/ите кои се предефинирани во
променливата emoticons доколку се појавуваат во документот).
"""
...
if __name__ == '__main__':
    ...
    emoticon_classifier = NaiveBayes(custom_get_words)

    # '.pop(index)' in 'list' removes the element at 'index' from the list caller and also returns the element as a result from the function call
    test_doc = data.pop(sample_ind)

    for doc in data:
        reg_classifier.train(doc[1], doc[0])
        emoticon_classifier.train(doc[1], doc[0])

    reg_pred = reg_classifier.classify_document(test_doc[1])
    emoticon_pred = emoticon_classifier.classify_document(test_doc[1])

    print(test_doc[1])
    print(f"Vistinska klasa: {test_doc[0]}")
    print(f"Klasa predvidena so Naiven Baes (bez emotikoni): {reg_pred}")
    print(f"Klasa predvidena so Naiven Baes (so emotikoni): {emoticon_pred}")