...
а втор елемент е класата како стринг. Да се истренира класификатор со користење на стандардната get_words
...
if __name__ == "__main__":
    ...
    reg_classifier = NaiveBayes(get_words)
    custom_classifier = NaiveBayes(get_words_with_ignore_2)

    for doc in train_data:
        reg_classifier.train(doc[0], doc[1])
        custom_classifier.train(doc[0], doc[1])

    reg_pred = reg_classifier.classify_document(recenica)
    custom_pred = custom_classifier.classify_document(recenica)

    print(reg_pred)
    print(custom_pred)

    if reg_pred != custom_pred:
        print("kontradikcija")
